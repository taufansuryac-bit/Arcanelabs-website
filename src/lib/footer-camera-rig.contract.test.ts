...
test("footer camera follows pointer direction smoothly and suppresses idle drift during interaction", () => {
  assert.match(footerSource, /const pointerResponse = 1 - Math\.exp\(-Math\.min\(delta, 0\.05\) \* 7\.2\)/);
  ...
  assert.match(footerSource, /const pointerMagnitude = Math\.min\(1, Math\.hypot\(state\.pointer\.x, state\.pointer\.y\)\)/);
  assert.match(footerSource, /const idleWeight = 1 - pointerMagnitude/);
  assert.match(footerSource, /lookTarget\.set\(cameraOffset\.current\.x \* 1\.55, 3\.8 \+ cameraOffset\.current\.y \* 1\.35/);
