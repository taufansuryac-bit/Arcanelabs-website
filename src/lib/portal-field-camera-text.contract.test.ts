...
test("voxel field camera follows pointer with restrained damped look parallax", () => {
  ...
  assert.match(source, /state\.pointer\.x \* 2\.0/);
  assert.match(source, /state\.pointer\.x \* 2\.4/);
  assert.match(source, /state\.pointer\.y \* 1\.3/);
  assert.match(source, /state\.pointer\.y \* 1\.6/);
  assert.match(source, /rollTarget = -state\.pointer\.x \* 0\.004 \* fieldInteraction/);
